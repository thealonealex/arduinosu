...
def convert_midi_to_arrays(filename):
    ...
    track = next(
        (t for t in mid.tracks if any(msg.type == "note_on" for msg in t)),
        mid.tracks[0],
    )
...
def backup_existing_file(filepath: Path):
        ...
        with open(filepath, "r") as f:
            ...
            for line in f:
                if "GENERATED FROM:" in line:
                    ...
                    midi_source_name = (
                        line.split(":")[1].strip().replace(".mid", "").strip()
                    )
                    break
    except Exception:
    ...
    while backup_filename.exists():
        ...
        backup_filename = original_backup_filename.with_stem(
            f"{original_backup_filename.stem}_{counter}"
        )
        counter += 1
    ...
    print(
        f"INFO: Existing file backed up to: '{backup_filename.name}' in the '{BACKUP_DIR}' folder."
    )
...
if __name__ == "__main__":
    ...
    if melody_macros and duration_ms:
        generate_header_file(
            melody_macros, duration_ms, OUTPUT_H_FILENAME, MIDI_FILENAME
        )
